...
public class XpUtils {
    ...
    public static double getStoredXpFromBlockEntity(BlockEntity be) {
        ...
        // Priority 1: Universal Accessor (Covers all modded and vanilla furnaces via UniversalBlockEntityMixin)
        if (isModSupportEnabled() && be instanceof TechRebornExperienceAccessor acc) {
            double universalXp = acc.furnaceXpTweaks$getExperience();
            if (universalXp > 0) return universalXp;
        }

        // Case 1: Vanilla / AbstractFurnaceBlockEntity (via Accessor for recipe map)
        if (be instanceof AbstractFurnaceBlockEntity furnace) {
            var recipes = ((AbstractFurnaceBlockEntityAccessor) furnace).furnaceXpTweaks$getRecipesUsed();
            if (be.getLevel() != null) {
                return calculateXpFromRecipesExact(recipes, be.getLevel().recipeAccess());
            }
        }
